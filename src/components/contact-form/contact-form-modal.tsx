      ...
      <Transition show={showModal} as={Fragment}>
        ...
        >
          ...
          <div className="fixed inset-0 z-[9999] flex items-center justify-center">
            ...
            >
              <Dialog.Panel
                className="absolute m-4 w-[92%] max-w-md overflow-y-auto rounded-[24px] border border-white/25 bg-gradient-to-br from-accent to-accent/90 px-5 py-5 shadow-[0_18px_60px_-28px_rgba(0,0,0,0.4)] backdrop-blur-2xl md:px-6 md:py-6"
                onClick={(event) => event.stopPropagation()}
              >
                <div className="flex items-center justify-between gap-3">
                  <Dialog.Title className="flex items-center gap-2 text-2xl font-semibold text-white sm:text-[30px]">
                    <MailIcon className="h-8 w-8 sm:h-9 sm:w-9" />
                    <span>Send Message</span>
                  </Dialog.Title>
                  <button
                    className="flex h-10 w-10 items-center justify-center rounded-full border border-white/30 bg-white/15 p-2 text-sm text-white shadow-md shadow-black/15 transition-transform duration-150 hover:scale-[1.04] hover:bg-white/25"
                    onClick={() => setShowModal(false)}
                  >
